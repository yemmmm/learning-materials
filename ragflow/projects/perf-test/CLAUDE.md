...
# Skip certain phases
python run_bench.py --skip-retrieval --skip-embedding --skip-monitor --skip-logs
```
...
Tests embedding API concurrency with short, controlled bursts to avoid triggering rate limits. Supports OpenAI-compatible and RAGFlow-internal API formats via `--api-format` (default: openai). Default: 5 concurrency, 20 requests. Includes warnings for high concurrency/request counts and concurrent queuing analysis (first-half vs second-half latency comparison).
...
```bash
# Install dependencies (uses parent repo's .venv at ../../.venv)
uv pip install -r requirements.txt

# Full interactive run
python run_bench.py

# Individual tools (all accept --config bench_config.json as alternative to CLI args)
...
python bench_embedding.py --api-url https://api.openai.com/v1/embeddings --api-key sk-xxx --model text-embedding-ada-002 --concurrency 5 --count 20 --output-json embedding.json
python bench_embedding.py --api-url http://localhost:18080/api/v1/embeddings --api-key ragflow-xxx --model bge-large-zh-v1.5 --concurrency 3 --count 10 --input "测试文本"
python analyze_logs.py --containers ha-node1-web ha-node2-web --since 5m --output-json analysis.json
python analyze_logs.py --config bench_config.json   # load params from config file
python plot_monitor.py -d ./bench-output -o ./plots
```

All individual tools support `--config` to load parameters from `bench_config.json` instead of specifying each CLI arg.
